...
namespace Instend.Core.Models.Storage.Collection
{
    public class Collection : AccessItemBase
    {
        ...
        public Configuration.CollectionTypes FolderType
        {
        ...
        public static Result<Collection> Create(string name, Guid ownerId, Guid folderId, Configuration.CollectionTypes folderType, bool visibility)
        {
            if (string.IsNullOrWhiteSpace(name) || string.IsNullOrEmpty(name))
                return Result.Failure<Collection>("Invalid folder name");
